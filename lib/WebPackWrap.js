...
class WebPackGenerator {
    ...
    static loadAPIPackage() {
        try {
            let basePath = require.resolve("@visualbi/powerbi-visuals-api", {
                paths: [process.cwd()]
            });
    ...
    async configureCustomVisualsWebpackPlugin(visualPackage, options, tsconfig) {
        ...
        try {
            apiVersionInstalled = (await exec('npm list @visualbi/powerbi-visuals-api version')).stdout.match(regexFullVersion)[0];
        } catch (err) {
            ConsoleWriter.warn(`"@visualbi/powerbi-visuals-api" is not installed`);
        }
        ...
        if (!apiVersionInstalled || (typeof this.pbiviz.apiVersion !== "undefined" && this.pbiviz.apiVersion.match(regexMinorVersion)[0] != apiVersionInstalled.match(regexMinorVersion)[0])) {
            ConsoleWriter.warn(`installed "@visualbi/powerbi-visuals-api" version - "${apiVersionInstalled}", is not match with the version specified in pbviz.json - "${this.pbiviz.apiVersion}".`);
