...
class PreProcessor {
   ...
   static processPbiviz(visualPackage, preOptions = {}) {
        ...
        // guid
        if(preOptions.customGuid) {
            visualPackage.config.visual.guid = preOptions.customGuid;
        }

        // display name with tag 
        let displayName = visualPackage.config.visual.displayName;
        let index = displayName.indexOf('(');        
        visualPackage.config.visual.displayName = displayName.slice(0, index).trim() + ' ' + `(${visualPackage.config.visual.version})`;
    }
